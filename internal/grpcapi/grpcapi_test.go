...
func TestAnalyze(t *testing.T) {
	tests := []struct {
		...
		{name: "safe JSON", service: app.New(analyzer.NewEngine(analyzer.DefaultRules()...)), input: `{"service":{"port":8080}}`, code: codes.OK, check: func(t *testing.T, response *cfgscanv1.AnalyzeResponse) {
			if len(response.GetProblems()) != 0 {
				t.Fatalf("problems = %#v, want none", response.GetProblems())
			}
		}},
		{name: "YAML finding", service: app.New(analyzer.NewEngine(analyzer.DefaultRules()...)), input: "database:\n  password: literal-password\n", code: codes.OK, check: func(t *testing.T, response *cfgscanv1.AnalyzeResponse) {
		...
		{name: "payload too large", service: app.New(analyzer.NewEngine()), input: strings.Repeat("a", maxConfigurationBytes+1), code: codes.ResourceExhausted},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			defer cleanup()
			response, err := client.Analyze(context.Background(), &cfgscanv1.AnalyzeRequest{Configuration: test.input})
